...
exports.getAvailableDrivers = async function (req, res) {
  ...
  Driver.find({ WorkingDates: { $ne: date } }).exec(function (err, drivers) {
    if (err) {
      res.status(400).json("Not success");
    } else {
      res.status(200).json(drivers);
...
//update vehicle total milage findby username
exports.updateVehicleTotalMilageByUsername = async function (req, res) {
  console.log(req.body);
  let username = req.params.username;
  let milage = req.body.distance;
  let driver = await Driver.findOne({ name: username });
  let vehicleTotalMilage = driver.milage + milage;
